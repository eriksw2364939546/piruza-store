...
function ManagerRow({ manager, onView }) {
  return (
    <tr
      className="amgr-row"
  ...
  return (
    <div className="amgr-page">
      <div className="amgr-page__head">
        ...
        {/* ❌ Нет кнопки создания — admin не может создавать менеджеров */}
      </div>
      ...
      {managers.length === 0 ? (
        ...
        <div className="amgr-page__table-wrap">
          <table className="amgr-page__table">
            ...
            <tbody>
              {managers.map((m) => (
                <ManagerRow key={m._id} manager={m} onView={setActiveManager} />
              ))}
            </tbody>
          </table>
        </div>
